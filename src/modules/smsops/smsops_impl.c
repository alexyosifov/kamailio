...
// 	SmallI_grave			= 7,	// EC -> i
// 	SmallO_grave			= 8,	// F2 -> o
// 	CapitalC_cedilla		= 9,	// C7 -> C
// 	CapitalO_stroke			= 11,	// D8 -> O
// 	SmallO_stroke			= 12,	// F8 -> o
// 	CapitalA_ring			= 14,	// C5 -> A
// 	SmallA_ring				= 15,	// E5 -> a
// 	Greek1					= 16,	// ?
// 	Underscore				= 17,	// 5F - OK
// 	Greek2					= 18,	// ?
...
// 	SmallAE					= 29,	// E6
// 	Small_eszett			= 30,	// DF
// 	CapitalE_acute			= 31,	// C9 -> E
// 	Currency_sign			= 36,	// A4
// 	Inverted_exclamaition	= 64,	// A1
// 	CapitalA_diaeresis		= 91,	// C4 -> A
// 	CapitalO_diaeresis		= 92,	// D6 -> O
// 	CapitalN_tilde			= 93,	// D1 -> N
// 	CapitalU_diaeresis		= 94,	// DC -> U
// 	Section_sign			= 95,	// A7
// 	Inverted_question		= 96,	// BF
// 	SmallA_diaeresis		= 123,	// E4 -> a
// 	SmallO_diaeresis		= 124,	// F6 -> o
// 	SmallN_tilde			= 125,	// F1 -> n
// 	SmallU_diaeresis		= 126,	// FC -> u
// 	SmallA_grave			= 127,	// E0 -> a
// };

// enum GSM_Extention_codes {
// 	Form_feed			= 10,	// 0C
// 	Circumflex			= 20,	// 5E
// 	Left_curly_bracket	= 40,	// 7B
// 	Right_curly_bracket	= 41,	// 7D
// 	Backslash			= 47,	// 5C
// 	Left_Square_bracket	= 60,	// 5B
// 	Tilde				= 61,	// 7E
// 	Right_Square_bracket= 62,	// 5D
// 	Vertical_bar		= 64,	// 7C
// 	Euro_sign			= 101,	// ?
// };
...
static int ascii_to_gsm(str sms, char * output_buffer, int buffer_size, unsigned char paddingBits) {
...
// //		output_buffer[output_buffer_length++] = sms.s[0] << (7 - carry_on_bits);
// 		output_buffer[output_buffer_length++] = ascii2gsm7bit_codes[(unsigned char)sms.s[0]] << (7 - carry_on_bits);
// 		carry_on_bits++;

		carry_on_bits = 7 - paddingBits;

		if (ascii2gsm7bit_codes[(unsigned char)sms.s[0]] == GSM7BIT_ESCAPE) {
			symbol = GSM7BIT_ESCAPE << (7 - carry_on_bits);
			symbol |= ascii2gsm7bit_ext_codes[(unsigned char)sms.s[0]] << (7 - carry_on_bits);
			carry_on_bits++;

			output_buffer[output_buffer_length++] = ascii2gsm7bit_ext_codes[(unsigned char)sms.s[i + 1]] << (7 - carry_on_bits);
		} else {
			output_buffer[output_buffer_length++] = ascii2gsm7bit_codes[(unsigned char)sms.s[0]] << (7 - carry_on_bits);
		}
...
// //		octet = (sms.s[i] & BITMASK_7BITS) >> carry_on_bits;
// 		symbol = (ascii2gsm7bit_codes[(unsigned char)sms.s[i]] & BITMASK_7BITS) >> carry_on_bits;

// 		if (i < sms.len - 1) {
// //			octet |= sms.s[i + 1] << (7 - carry_on_bits);
// 			symbol |= ascii2gsm7bit_codes[(unsigned char)sms.s[i + 1]] << (7 - carry_on_bits);
// 		}
...
// 		carry_on_bits++;
		...
		if (ascii2gsm7bit_codes[(unsigned char)sms.s[i]] == GSM7BIT_ESCAPE) {
			...
			if (i < sms.len - 1) {
				symbol |= ascii2gsm7bit_ext_codes[(unsigned char)sms.s[i + 1]] << (7 - carry_on_bits);
			}

			output_buffer[output_buffer_length++] = symbol;
			carry_on_bits++;

			symbol = ascii2gsm7bit_ext_codes[(unsigned char)sms.s[++i]] << (7 - carry_on_bits);
		} else {
			symbol = (ascii2gsm7bit_codes[(unsigned char)sms.s[i]] & BITMASK_7BITS) >> carry_on_bits;
		}
...
//				LM_CRIT("==========1========== symbol=0x%02X, %c, %c, 0x%02X\n", symbol, sms.s[output_text_length - 1], gsm7bit_codes[symbol], gsm7bit_codes[symbol]);
			}else{
//				LM_CRIT("==========ESCAPE 1==========\n");
				isEscape = 1;
			}
...
//				LM_CRIT("==========extended %d ========== symbol=0x%02X, %c, %c, 0x%02X\n", i, symbol, sms.s[output_text_length - 1], gsm7bit_ext_codes[symbol], gsm7bit_ext_codes[symbol]);
			} else {
				if (symbol != GSM7BIT_ESCAPE) {
					sms.s[output_text_length++] = gsm7bit_codes[symbol];
//					LM_CRIT("==========%d========== symbol=0x%02X, %c, %c, 0x%02X\n", i, symbol, sms.s[output_text_length - 1], gsm7bit_codes[symbol], gsm7bit_codes[symbol]);
				} else {
//					LM_CRIT("==========ESCAPE %d==========\n", i);
					isEscape = 1;
				}
...
int pv_sms_body(struct sip_msg *msg, pv_param_t *param, pv_value_t *res) {
	...
	if (rp_send_data->pdu.coding == 0x00) {
		...
		i = ascii_to_gsm(rp_send_data->pdu.payload.sm, &sms_body.s[sms_body.len], buffer_size - sms_body.len, paddingBits);
		sms_body.len += i;
		sms_body.s[smstext_len_pos] += udh_len;
	} else {
	// Coding: ucs2
